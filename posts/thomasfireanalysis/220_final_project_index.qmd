...
At this point, our `landcover` `xarray.DataArray` is multidimensional. We'll drop the `band` dimension and its variables, just as we did before:
...
The first line finds the counts of unique values in the `landcover_clip` `xarray.DataArray`.
